...
export default function ContributionShooter({
  ...
  const [gameState, setGameState] = useState<"START" | "PLAYING" | "VICTORY">(
    "START",
  );
  ...
  useEffect(() => {
    ...
    const initBlocks = async () => {
      try {
        ...
        try {
          ...
          // Fetch both years because the rolling 12-month
          // calendar crosses the year boundary.
          ...
          const previousData = previousRes.ok
            ? await previousRes.json()
            : { contributions: [] };

          contributions = [
            ...(previousData.contributions || []),
            ...(currentData.contributions || []),
          ];

          // Remove duplicate dates and create a lookup map.
          // Group into Sunday -> Saturday weeks
          ...
          // Keep approximately one year of GitHub-style weeks
          const selectedWeeks = weeks.slice(-53);

          // Smaller blocks so the complete year fits inside
          // the existing 900px canvas.
          const blockW = 13;
          const blockH = 13;
          const gap = 3;
        ...
        const selectedWeeks = weeks.slice(-52);
    ...
    const loop = () => {
          ...
          for (const block of blocksRef.current) {
            ...
            if (
              ...
              if (block.hp <= 0) {
                ...
                setScore(
                  (prev) =>
                    prev + (block.isCommit ? (block.level + 1) * 50 : 10),
                );
              } else {
                sound.playHit();
              }
      ...
      if (stateRef.current === "START") {
        ...
        ctx.fillText(
          "Shoot through real GitHub contributions with laser fire!",
          450,
          280,
        );
  ...
  return (
    <div className="relative mx-auto w-full max-w-4xl overflow-hidden rounded-2xl border border-emerald-500/30 bg-[#050810] shadow-[0_0_40px_rgba(16,185,129,0.15)]">
      ...
      <canvas
        ...
        className="block aspect-[3/2] w-full cursor-crosshair"
      />

      {/* Mobile Touch Controls */}
      <div className="flex items-center justify-between border-t border-white/10 bg-black/60 p-4 sm:hidden">
